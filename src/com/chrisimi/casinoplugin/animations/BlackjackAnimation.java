...
/**
 * animation class for blackjack signs
 */
public class BlackjackAnimation implements Runnable
{
    ...
    private static final HashMap<Player, BlackjackAnimation> waitingForInputs = new HashMap<Player, BlackjackAnimation>();
    ...
    public BlackjackAnimation(PlayerSignsConfiguration thisSign, Player player, PlayerSignsManager manager)
    {
...
//		
        ...
        currentWaitingTask = Main.getInstance().getServer().getScheduler().runTaskLater(Main.getInstance(), new Runnable()
        {
            @Override
            public void run()
            {
        ...
        resetTask = Main.getInstance().getServer().getScheduler().runTaskLater(Main.getInstance(), new Runnable()
        {
            @Override
            public void run()
            {
    ...
    public void userBetInput(Double input)
    {
        //the start bet
    ...
    {
        //TODO implement user split cards
    }
    ...
    private void finish()
    {
        ...
        finished = true;
        Main.getInstance().getServer().getScheduler().cancelTask(resetTask);
    }

    private void resetSign()
    {
        //animation is "finished" and bring back to normal
        if (waitingForInputs.containsKey(player))
            waitingForInputs.remove(player);
    ...
    {
        //show the player which card he has and what he can do next
    ...
    {
        //at the next round the dealer should made this action! (simulated) return if dealer wons/lose
        int valueOfDealer = Card.getValue(dealer);

        if (!(Card.getValue(cards) > 21)) //only calc dealer values when the player could possibly win
    ...
    {
        //before the next round started check if the dealer or the player could win the game!
        int valueOfDealer = Card.getValue(dealer);
    ...
    { //change the sign to the new values
        String a = "";
        for (Card card : cards)
            a += card.toString() + ", ";
    ...
    public static void userInput(String message, Player player)
    {
        ...
        if (thisAnimation.waitingForBet)
        {
            ...
            Main.getInstance().getServer().getScheduler().cancelTask(thisAnimation.currentWaitingTask);
            waitingForInputs.remove(player);
